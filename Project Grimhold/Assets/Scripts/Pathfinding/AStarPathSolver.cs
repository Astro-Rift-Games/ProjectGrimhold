...
public sealed class AStarPathSolver
{
    ...
    /// Agent radius used during path smoothing (CircleCast). Must match the
    /// value used when the grid was built.
    /// </param>
        ...
        List<Vector2> outputWaypoints)
    {
        ...
        while (_openHeap.Count > 0 && iterations < maxIterations)
        {
            ...
            if (cx == ex && cy == ey)
            {
                ...
                SmoothPath(outputWaypoints, agentRadius, endWorld);
    ...
    /// </summary>
    private void SmoothPath(List<Vector2> waypoints, float agentRadius, Vector2 endWorld)
    {
        ...
        while (current < waypoints.Count - 1)
        {
            ...
            {
                if (HasDirectPath(waypoints[current], waypoints[lookahead], agentRadius))
                {
                    furthest = lookahead;
                }
            }
    ...
    /// obstacle collider.
    /// </summary>
    private bool HasDirectPath(Vector2 from, Vector2 to, float agentRadius)
    {
