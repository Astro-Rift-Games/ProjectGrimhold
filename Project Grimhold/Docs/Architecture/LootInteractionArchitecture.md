...
| --- | ---: | ---: | ---: | ---: |
| `NetworkPlayer` | 1289 | 1075 | -214 | <= 1611 |
| `LootContainer` | 820 | 551 | -269 | <= 1025 |
| `NetworkEnemy` | 844 | 575 | -269 | <= 1055 |
| `NetworkLootPickup` | 6 | 22 | +16 | <= 192 |

The approved comparison retained full logical capacity. Direct `RaidParticipantId` records weave to 817 words for Inventory/container, 827 for Player including eight Equipment references, and 35 for Pickup. A local table plus packed indices weighs 383, 384 and 32 words. Encoding the logical ID directly in the packed origin field is smallest at 367, 369 and 16 words and is the selected representation. TASK-144 adds four words to every prefab composing `NetworkLootContainer`: the terminal state, two seed words and effective probability. TASK-330 adds two replicated hand indices and expands packed Equipment provenance from 30 to 40 bits. `RaidLootOriginWordCountTests` provides reproducible verification of the baked budgets. The previous `RaidProfileIdKey` and `NetworkString<_32>` candidates were removed after this contract was validated.
