...
- **Migration Status**:
  ...
  - **HM-04: Runner-local completion barrier**:
    ...
    - `NetworkSpawnManager` exists before `StartGame` and owns early arrivals plus the complete runner-scoped recovery roster keyed by `ProfileId`.
    - Only `GameMode.Host && IsServer` executes snapshot restore, atomic roster sealing, unrecovered finalization, and authoritative completion. `GameMode.Client && !IsServer` waits only for its replicated local mapping and never enters server completion.
    - Each process has a local 65-second lifecycle budget. Startup/scene/restore is capped at 30 seconds and the Host recovery window is capped at a further 30 seconds within the remaining budget.
    - Snapshot success alone is not terminal. Host completion requires a sealed roster in which every `Raiding` participant has an active peer, PlayerObject, and avatar authority, while every unrecovered profile is terminal and non-operational.
    - Full ownership, state-machine, finalization, and validation rules are documented in `Docs/Architecture/HostMigrationRecoveryArchitecture.md`.
  ...
  - **HM-05: Edge Cases & Graceful Aborts**:
    ...
    - **Unsupported prior phase**: A callback whose source Raid was already `Closing` or `Finished` is rejected. `Closing` caused after a valid `InProgress` recovery by natural zero-participant completion is valid.
  ...
  **Important Distinctions**:
  ...
  - **Defeated Host remains Host**: While connected, the operational Host is the runner server/local player even when its avatar is defeated. Return, Abandon and Cancel Raid are rejected; Host Migration remains reserved for actual Host peer loss. The frozen `RaidLaunchContext.HostProfileId` identifies the historical Host that must not reconnect after replacement.
  ...
  - **Runner ownership handoff**: `HostMigrationLifecycleController` creates and starts the replacement composition, but `FusionSessionLauncher` remains the sole long-lived owner. `StartGame` returning successfully does not authorize adoption. A Host is adopted only after server completion; a Client is adopted after its own local mapping is valid. Until that point the launcher never exposes the replacement as active.
