...
- **Migration Status**:
  ...
  - **HM-05: Edge Cases & Graceful Aborts**:
    - **Lobby Disconnects**: If the original Host disconnects prematurely and the cloud delivers an outdated snapshot in the `WaitingForPlayers` phase, the validation fails in a controlled manner executing a *Graceful Abort*: it performs a local *Rollback*, shuts down the Runner, and returns surviving clients to the `MainMenu`, preventing *soft-locks*.
